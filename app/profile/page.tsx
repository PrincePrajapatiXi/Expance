...
export default function ProfilePage() {
  ...
  const handleExportExcel = () => {
    ...
    setMessage('Exported transactions ledger to Excel (.xlsx) file.');
    setTimeout(() => setMessage(null), 3000);
  };

  const handleExportCSV = () => {
    exportTransactionsToCSV(transactions);
    setMessage('Exported transactions ledger to CSV file.');
    setTimeout(() => setMessage(null), 3000);
  };
    ...
    <div className="min-h-screen bg-slate-50 dark:bg-slate-950 text-slate-900 dark:text-slate-100 pb-24">
      ...
      <main className="max-w-4xl mx-auto px-4 pt-2">
        ...
        <div className="my-2">
          ...
          <p className="text-xs text-slate-500">Manage profile, cloud database sync, import, and backup</p>
        </div>
        ...
        <div className="bg-white dark:bg-slate-900 rounded-3xl p-5 border border-slate-200/80 dark:border-slate-800 shadow-xs mb-4">
          <div className="flex items-center justify-between pb-3 border-b border-slate-100 dark:border-slate-800 mb-3">
            <div className="flex items-center gap-2">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">Bulk upload or download ledger spreadsheets</p>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            ...
            <button
              ...
              className="p-4 rounded-2xl bg-indigo-50 hover:bg-indigo-100 dark:bg-indigo-950/40 dark:hover:bg-indigo-950/60 text-indigo-900 dark:text-indigo-300 font-extrabold text-xs flex items-center justify-between border border-indigo-200/70 dark:border-indigo-800 transition-all cursor-pointer"
            >
              <div className="flex items-center gap-3">
                <UploadCloud className="w-6 h-6 text-indigo-700 dark:text-indigo-400" />
                <div className="text-left">
                  <span>Import CSV / Excel</span>
                  <span className="block text-[10px] text-indigo-700/80 dark:text-indigo-400/80 font-medium">
            ...
            <button
              ...
              className="p-4 rounded-2xl bg-emerald-50 hover:bg-emerald-100 dark:bg-emerald-950/40 dark:hover:bg-emerald-950/60 text-emerald-900 dark:text-emerald-300 font-extrabold text-xs flex items-center justify-between border border-emerald-200/70 dark:border-emerald-800 transition-all cursor-pointer"
            >
              <div className="flex items-center gap-3">
                <FileSpreadsheet className="w-6 h-6 text-emerald-700 dark:text-emerald-400" />
                <div className="text-left">
                  <span>Export to Excel (.xlsx)</span>
                  <span className="block text-[10px] text-emerald-700/80 dark:text-emerald-400/80 font-medium">
                    Spreadsheet with ledger
                  </span>
                </div>
              </div>
            ...
            <button
              ...
              className="p-4 rounded-2xl bg-sky-50 hover:bg-sky-100 dark:bg-sky-950/40 dark:hover:bg-sky-950/60 text-sky-900 dark:text-sky-300 font-extrabold text-xs flex items-center justify-between border border-sky-200/70 dark:border-sky-800 transition-all cursor-pointer"
            >
              <div className="flex items-center gap-3">
                <FileCode className="w-6 h-6 text-sky-700 dark:text-sky-400" />
                <div className="text-left">
                  <span>Export to CSV</span>
                  <span className="block text-[10px] text-sky-700/80 dark:text-sky-400/80 font-medium">
                    Plain comma-separated
                  </span>
                </div>
              </div>
              <Download className="w-4 h-4 text-sky-700 dark:text-sky-400" />
            </button>
          </div>
        </div>
