...
export default function TransactionModal({
  ...
  // Populate form values when editing or opening
  useEffect(() => {
    if (!isOpen) return;
  ...
  const handleDragEnd = (_: any, info: PanInfo) => {
    // If pulled down far enough or fast enough, dismiss bottom sheet with haptic feedback
  ...
  return (
    <AnimatePresence>
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4 pointer-events-auto select-none">
          {/* Backdrop */}
          <motion.div
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            ...
            className="fixed inset-0 bg-slate-900/60 backdrop-blur-xs cursor-pointer z-0"
          />
          ...
          <motion.div
            ...
            className="relative z-10 w-full sm:max-w-lg bg-white dark:bg-slate-900 rounded-t-[32px] sm:rounded-3xl p-5 sm:p-6 shadow-2xl border-t sm:border border-slate-200/90 dark:border-slate-800 max-h-[92vh] sm:max-h-[88vh] flex flex-col overflow-hidden pb-[calc(1.25rem+env(safe-area-inset-bottom,0px))] sm:pb-6"
          >
            {/* Native Mobile Swipe-down Pill Handle */}
            <div className="flex justify-center pb-2 cursor-grab active:cursor-grabbing sm:hidden touch-none">
            ...
            <div className="flex items-center justify-between pb-3.5 border-b border-slate-100 dark:border-slate-800 shrink-0">
              ...
              <button
                ...
                className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 hover:text-slate-600 transition-transform duration-100 ease-out active:scale-90 cursor-pointer"
            ...
            <form onSubmit={handleSubmit(handleFormSubmit)} className="space-y-4 mt-3 overflow-y-auto pr-1 no-scrollbar">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 gap-2 p-1.5 bg-slate-100 dark:bg-slate-800/80 rounded-2xl border border-slate-200/60 dark:border-slate-700/60">
                  ...
                  <button
                    ...
                    className={`py-2.5 rounded-xl font-extrabold text-xs transition-transform duration-100 ease-out active:scale-95 flex items-center justify-center gap-1.5 cursor-pointer ${
                      selectedType === 'income'
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                  {paymentModes.map((mode) => (
                    <button
                      ...
                      onClick={() => {
                        triggerHaptic(12);
                        setValue('payment_method', mode.id);
                      }}
                      className={`p-2.5 rounded-xl text-xs font-bold transition-transform duration-100 ease-out active:scale-95 cursor-pointer ${
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 gap-2 max-h-36 overflow-y-auto p-1 border border-slate-200/60 dark:border-slate-800 rounded-2xl">
                  {availableCategories.map((cat) => (
                    <button
                      ...
                      className={`p-2 rounded-xl text-left text-xs font-semibold flex items-center gap-2 border transition-transform duration-100 ease-out active:scale-95 cursor-pointer ${
              ...
              {/* Submit Button with Native Haptic feedback & active spring animation */}
              <div className="pt-2">
                <button
                  type="submit"
                  disabled={isSubmitting}
                  className="w-full py-3.5 rounded-2xl bg-indigo-900 hover:bg-indigo-950 text-white font-extrabold text-sm transition-transform duration-100 ease-out shadow-lg shadow-indigo-900/20 active:scale-95 cursor-pointer flex items-center justify-center gap-2"
                >
